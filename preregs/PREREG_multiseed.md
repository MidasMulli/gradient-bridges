...
0.38 to 0.53 alignment a property of the pinned frame (per-frame determinism) or an
artifact of seed 7102?

## Design
NEW SEED S2 = 3141 (single new seed, pinned across all S2 targets, which preserves the
...
falls in [0.30, 0.60] (consistent with S1's 0.38 to 0.53). COLLAPSE (mean < 0.15 or
...
cos_W(ΔW_branch_S1(T), ΔW_branch_S2(T)) for the 6 matched tickers vs cross-ticker null.
 ...
 - ≈ null ⇒ solutions are seed-idiosyncratic HERE TOO, but (if P1 confirms) paths are
   per-frame predictable, which reconciles our breach with Mac's null under one
   mechanism: grad-at-init works only in its own frame; their Route-6 used a frame-free
   basis.
